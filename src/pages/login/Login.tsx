...
export default function Login() {
  ...
  return (
    <main className="relative flex h-full min-h-0 flex-col overflow-y-auto bg-background-50">
      {/* 페이지 상단 로고와 안내 문구 영역 */}
      <section className="px-6 pt-20">
        <img src={icon} alt="PenFit" className="mb-7 h-12 w-12 rounded-xl" />

        <h1 className="mb-3 font-heading text-xl font-bold text-foreground-950">
          시작해볼까요?
        </h1>
      ...
      <section className="flex flex-col gap-4 px-6 pb-8">
        ...
        <div className="flex items-start gap-2">
          <input
            ...
            className="mt-0.5 h-4 w-4 cursor-pointer rounded border-background-300 accent-primary-500"
          />
          <label
            htmlFor="agree"
            className="cursor-pointer text-xs leading-relaxed text-foreground-700"
          >
        ...
        <button
          ...
          className="w-fit pl-6 text-left text-xs text-foreground-600 underline transition-colors hover:text-foreground-800"
        >
          약관 자세히 보기
        </button>

        {/* 실제 금융상품 가입이 아닌 가상 체험임을 알리는 문구 */}
        <p className="pt-3 text-center text-[10px] leading-relaxed text-foreground-400">
      ...
      {showTerms && (
        <div className="absolute inset-0 z-50 flex items-end bg-black/40 animate-fade-in">
          <section className="flex max-h-[80%] w-full flex-col rounded-t-2xl bg-background-50 p-6">
            <div className="mb-4 flex items-start justify-between gap-4">
              ...
              <button
                ...
                className="flex h-8 w-8 shrink-0 items-center justify-center text-xl text-foreground-600"
                aria-label="약관 닫기"
              >
                x
              </button>
            ...
            <div className="flex-1 space-y-3 overflow-y-auto text-sm leading-relaxed text-foreground-700">
              <p className="font-semibold text-foreground-950">제1조 (목적)</p>
              <p>
                본 약관은 PenFit 서비스의 이용 조건 및 절차, 회사와 회원 간의 권리,
                의무 및 책임사항 등을 규정함을 목적으로 합니다.
              </p>
              <p className="font-semibold text-foreground-950">제2조 (서비스의 성격)</p>
              <p>
                본 서비스는 가상 체험 서비스로, 실제 금융상품 가입을 대행하거나
                중개하지 않습니다. 서비스 내의 모든 연금 시뮬레이션 결과는 참고용입니다.
              </p>
              <p className="font-semibold text-foreground-950">
                제3조 (개인정보의 수집 및 이용)
              </p>
              <p>
